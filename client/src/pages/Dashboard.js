...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    try {
      const [statsRes, locationsRes, servicesRes, serviceTypesRes] = await Promise.all([
    ...
    <div>
      <div className="card">
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: "20px" }}>
          <div className="card" style={{ background: "linear-gradient(135deg, #3498db, #2980b9)", color: "white" }}>
            <h3 style={{ margin: "0 0 10px 0", fontSize: "16px" }}>Total Locations</h3>
            <p style={{ margin: 0, fontSize: "32px", fontWeight: "bold" }}>{stats.totalLocations}</p>
          </div>

          <div className="card" style={{ background: "linear-gradient(135deg, #27ae60, #229954)", color: "white" }}>
            <h3 style={{ margin: "0 0 10px 0", fontSize: "16px" }}>Total Services</h3>
            <p style={{ margin: 0, fontSize: "32px", fontWeight: "bold" }}>{stats.totalServices}</p>
          </div>

          <div className="card" style={{ background: "linear-gradient(135deg, #e74c3c, #c0392b)", color: "white" }}>
            <h3 style={{ margin: "0 0 10px 0", fontSize: "16px" }}>Service Types</h3>
            <p style={{ margin: 0, fontSize: "32px", fontWeight: "bold" }}>{stats.totalServiceTypes}</p>
          </div>
        </div>
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: "flex", gap: "15px", flexWrap: "wrap" }}>
          ...
          <button className="btn btn-success" onClick={() => navigate("/services")}>
            ⚙️ Create Service
          </button>
          <button className="btn btn-secondary" onClick={() => navigate("/network-map")}>
            🗺️ View Network Map
          </button>
        </div>
      </div>

      <NetworkAnalytics locations={recentLocations} serviceTypes={serviceTypes} services={services} />
